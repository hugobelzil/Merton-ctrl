...
Several experiments were conducted to test the method on the loss of $\beta$ -dTD in the infinite horizon setting. Our first empirical findings were that using small values of $K$ does not consistently help the $\beta$-dTD to achieve better results. Here, $K=32$ samples were used at each transition. The results reported in table \ref{tab:beta_dtd_results} show that one noticeable improvement happens where $\beta=0.9$ where the loss essentially consists of the dTD component. However, and  in regards of the results with higher values of $K$ in further sections, our conclusion is that 32 samples are not enough to harness the large variance of the dTD estimator.
