...
For this choice of utility function, there is a closed-form solution (\cite{merton1971optimum}). This is extremely useful, as it allows us to directly assess the quality of the learned value function, rather than relying on the evolution of the reward over training, as was necessary in the Brax experiments (where no closed-form solution exists).\TODO{add a brief appendix with the formulas?}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\linewidth]{media/dtd_value_fit.png}
    \caption{Value function learned with the pure dTD loss}
    \label{fig:collapse}
\end{figure}

The cause of this behavior was already outlined earlier. The drift estimator in the dTD residual (Equation \eqref{eq:residual}) is asymptotically unbiased but highly noisy. More precisely, the prediction term in the loss is given by
...
so the noise in the estimator grows unboundedly as $\Delta t \to 0$ and as $\sigma$ increases. This drives the optimization toward a spurious attractor at $\nabla V \to 0$, yielding a constant value function. To test this hypothesis, we ran $\beta$-dTD across a range of hyperparameter configurations (see Figure \ref{fig:merton_sweep}). The results confirm that the noise problem is more pronounced in high-diffusion settings, consistent with Equation \eqref{eq:noise}. Moreover, in the cases where $\beta$-dTD does not collapse, we are unable to confirm the claim of \cite{settai2025} that $\beta$-dTD exhibits a faster rate of improvement than standard TD (see Figure \ref{fig:faster_rate}).
...
\section{Analytical solution for the Merton Problem}
...
\begin{figure}
    ...
    \centering
    \includegraphics[width=0.4\textwidth]{media/pretrained_dtd.jpeg}
    \caption{MAE for learned value function starting from a pre-trained network}
\end{figure}
